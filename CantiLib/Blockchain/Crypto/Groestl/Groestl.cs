...
namespace Canti.Blockchain.Crypto.Groestl
{
    public static class Groestl
    {
        public static byte[] groestl(byte[] input)
        {
            ...
            int l = Constants.bufferSize * 8;
            ulong N = 0;
            ulong t = 0;
            ulong T = 0;
            ulong w = 0;

            byte[] h = new byte[Constants.bufferSize];
            System.Array.Copy(Constants.IV, h, Constants.bufferSize);
            ...
            for (int i = 0; i < input.Length; i+= Constants.bufferSize)
            {
                /* Can't read a whole block */
                if ((input.Length - i) < Constants.bufferSize)
                {
                ...
                Buffer.BlockCopy(input, i, buffer, 0, Constants.bufferSize);

                N = N + (ulong)((Constants.bufferSize * 8));
            ...
            int numberOfZeroBytes = Constants.bufferSize - bytesRead - 1;
            ...
            for (uint i = 0; i < blocksLeft; i++)
            {
                if (i == blocksLeft - 1)
                {
                    ...
                    System.Array.Copy(bytes, 0, buffer, Constants.bufferSize - 8, 8);
                }
